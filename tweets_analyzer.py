...
def print_stats(dataset, top=10):
...
def main():
    ...
    cprint("[+] Detected languages (top 10)")
    ...
    if not args.no_retweets:
        ...
        cprint("[+] Top 10 most retweeted users")
        print_stats(retweeted_users_names, top=10)
    ...
    cprint("[+] Top 10 most mentioned users")
    print_stats(mentioned_users_names, top=10)
    jsono["top_mentioned_users"] = mentioned_users_names

    cprint("[+] Most referenced domains (from URLs)")
    print_stats(detected_domains, top=10)
    ...
    if args.friends:
        ...
        print_stats(friends_lang, top=10)
        jsono["top_friends_languages"] = friends_lang

        cprint("[+] Friends timezones")
        print_stats(friends_timezone, top=10)
